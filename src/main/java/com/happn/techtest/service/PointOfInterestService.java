...
public class PointOfInterestService {
    ...
    public long getNumberOfPOIInZone (List<PointOfInterest> pois, double minLat, double minLon) {
        return pois.stream()
                   .filter(point -> point.getLat() > minLat && point.getLon() > minLon)
                   .count();
    }

    /**
     * Retrieve the N densest zone from a list of {@link PointOfInterest}. Start by creating a Map containing for each
     * zone the number of {@link PointOfInterest} in it. We then store in a bucket (an array of list) the
     * PointOfInterest that appears at each frequency. This gives us the ordered list but in reverse order. That's why
     * we iterate from end to start in order to return the N densest zone. More explanations in the README file.
    ...
    public List<GridZone> getNDensestZone(List<PointOfInterest> pois, int n) {
        ...
        // if points are very compact, we can have less zones than required with n so in order to avoid exceptions, we
